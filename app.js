...
function kpiCard(value, label, icon, tone, trend) {
  return `<article class="card kpi ${tone}"><div class="kpi-icon">${icon}</div><span class="eyebrow">${escapeHtml(label)}</span><strong>${Number(value || 0).toLocaleString()}</strong><label>${escapeHtml(label)}</label><span class="trend ${trend < 0 ? 'warning' : ''}">${trend == null ? 'Live state' : `${trend >= 0 ? '↑' : '↓'} ${Math.abs(trend).toFixed(0)}% vs. previous snapshot`}</span></article>`;
}
...
function renderDistribution(state) {
  ...
  const entries = [['green',counts.green||0,'#2aa56b'],['half_ripened',counts.half_ripened||0,'#e39a1c'],['fully_ripened',counts.fully_ripened||0,'#df574c']];
  let cursor = 0; const segments = entries.map(([,n,c]) => { const start = cursor; cursor += total ? n/total*100 : 0; return `${c} ${start}% ${cursor}%`; });
  $('#donut').style.background = `conic-gradient(${segments.join(',') || '#e6eee9 0 100%'})`;
...
function renderHealth(state) {
  ...
  $('#health-ring').style.background = `conic-gradient(var(--green) ${h.overall}%,#e8efeb ${h.overall}% 100%)`;
